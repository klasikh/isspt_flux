...
//   const { user } = useUser()
  const { data, loading, error, fetchMore } = useQuery(AllFilieresQuery, {
    variables: { first: 10 },
  });
  ...
  const { endCursor, hasNextPage } = data?.filieres.pageInfo;
          ...
          <Card>
            <CardHeader variant="gradient" color="blue" className="mb-8 p-6">
              <Typography variant="h6" color="white">
                Filière ({data?.filieres.edges.length})
              </Typography>
            </CardHeader>
            <CardBody className="overflow-x-scroll px-0 pt-0 pb-2">
              <table className="w-full min-w-[640px] table-auto">
                ...
                <tbody>
                  { data?.filieres.edges.map(({ node }: { node: Node }) => (
                        <tr key={node.id}>
                          ...
                          <td className={`py-3 px-5`}>
                            ...
                            >
                              ...
                              <Link href={`/filieres/delete/${node.id}`}>
