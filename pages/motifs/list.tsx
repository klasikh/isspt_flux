...
const DeleteMotifMutation = `
  mutation($id: ID!, $userId: ID!,) {
    deleteMotif(id: $id, userId: $userId,) {
      id
    }
  }
`
...
const MotifsList = ({ motifs }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
    ...
    const [motifToDel, setMotifToDel] = useState()
    ...
    const delMotif = async () => {
      setIsDeleteLoading(true);
      const variables = { id: motifToDel, userId: session?.user.id, }
      try {

        const theDeletedMotif =  await axios.post('/api/graphql', {
                                        "query": DeleteMotifMutation,
                                        "variables" : variables
                                        },
                                    { headers: { 'Content-Type': 'application/json' } }
                                    );

        if(theDeletedMotif?.data.errors) {
          toast.error(`${theDeletedMotif?.data.errors[0].extensions.originalError.message}`)
                        ...
                        <tr key={node.id}>
                          ...
                          <td className={`py-3 px-5`}>
                            ...
                            >
                              ...
                              <Link href={`#`} onClick={() => deleteClickMotif(node.id)}>
                        ...
                        <div className="mt-3 text-center sm:ml-4 sm:mt-0 sm:text-left">
                          ...
                          <div className="mt-2">
                            <p className="text-sm text-gray-700">
                              Êtes-vous sûr(e) de vouloir supprimer ce motif ?
                            </p>
