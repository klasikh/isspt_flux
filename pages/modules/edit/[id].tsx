...
const EditModule = ({ moduled }: InferGetServerSidePropsType<typeof getServerSideProps>) => {
  ...
  const [editModule, { data, loading, error }] = useMutation(EditModuleMutation)
  ...
  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    ...
    const variables = { id: moduled.id, name, description, }
    try {
      const theEditedmodule = await toast.promise(editModule({ variables }), {
        loading: 'Opération en cours..',
        success: 'Module modifié avec succès!🎉',
        error: `Une erreur s'est produite 😥 Veuillez re-essayer SVP - ${error}`,
      })

      if(theEditedmodule.data.updateModule) {
        router.push(`/modules/${theEditedmodule.data.updateModule.id}`)
      }
  ...
  return (
    <div>
      ...
      <div className="container mx-auto max-w-lg mt-10">
        ...
        <h1 className="text-3xl font-medium mb-5 uppercase text-center">Modifier un module</h1>
        <form className="grid grid-cols-1 gap-y-6 bg-white shadow-lg p-8 rounded-lg" onSubmit={handleSubmit(onSubmit)}>
          ...
          <label for="description" class="block">
            ...
            <textarea id="description" rows="4" {...register('description', { required: true })} required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" placeholder="Description du module" name="description"></textarea>
          </label>
